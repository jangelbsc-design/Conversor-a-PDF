"""Router de reordenamiento de páginas."""
import uuid
...
from app.services.pdf_reorder import reorder_pages
...
async def reorder_document_pages(
    ...
    new_order: Annotated[list[int], Body(embed=True, min_length=1)],
    db: AsyncSession = Depends(get_db),
):
    """
    Reordena páginas de un PDF.

    new_order: [3, 1, 2] → la pág 3 queda primera, luego la 1, luego la 2.
    Puede incluir duplicados para repetir páginas.
    """
    ...
    op = Operation(
        ...
        params_json={"new_order": new_order},
    ...
    out_path = out_dir / f"reordered_{ts}.pdf"

    try:
        reorder_pages(in_path, out_path, new_order)
        ...
        op.completed_at = datetime.now(timezone.utc)
        raise HTTPException(status_code=500, detail=f"Error al reordenar: {e}")

    return op


@router.get("/{operation_id}/download")
async def download_reordered(operation_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    op = await db.get(Operation, operation_id)
    if not op or op.operation_type != OperationType.reorder:
